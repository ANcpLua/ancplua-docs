...
**Null-safety improvements** - Comprehensive nullability annotations throughout the library:

- `[MemberNotNullWhen]` on `DiagnosticFlow.IsSuccess` for flow-dependent null state
- `[MaybeNullWhen(false)]` for `TryGet` patterns
- `[return: MaybeNull]` for `GetOrInsertDefault`
- Null-forgiving operators reduced from 13 to 1
