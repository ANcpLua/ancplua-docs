...
```csharp
using ANcpLua.Roslyn.Utilities.Testing.WebTesting.NUnit;
...
<Note>
  NUnit version uses `[OneTimeSetUp]` for factory creation (shared across tests)
  and `[SetUp]`/`[TearDown]` for per-test client lifecycle.
</Note>

### IntegrationTestBase (TUnit)

```csharp
using ANcpLua.Roslyn.Utilities.Testing.WebTesting.TUnit;
...
<Note>
  TUnit version uses `[Before(HookType.Test)]` / `[After(HookType.Test)]` hooks.
  A fresh factory and client are created per test.
</Note>
...
```csharp
using ANcpLua.Roslyn.Utilities.Testing.WebTesting.Bunit;
...
public class MyComponentTests : BunitTestBase
{
    ...
    public void Counter_Increments()
    {
        ...
        // Assert on fake logs
        Logs.ShouldContain("Counter incremented");
    }
}
```

<Note>
  `BunitTestBase` extends `BunitContext` and pre-configures `FakeLogCollector`.
  Works with any test framework (xUnit, NUnit, TUnit).
</Note>
...
| Framework | Namespace                                           |
| --------- | --------------------------------------------------- |
| xUnit     | `ANcpLua.Roslyn.Utilities.Testing.WebTesting`       |
| NUnit     | `ANcpLua.Roslyn.Utilities.Testing.WebTesting.NUnit` |
| TUnit     | `ANcpLua.Roslyn.Utilities.Testing.WebTesting.TUnit` |
| bUnit     | `ANcpLua.Roslyn.Utilities.Testing.WebTesting.Bunit` |
