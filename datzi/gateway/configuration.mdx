...
title: Configuration
summary: Overview of how to configure the Datzi Gateway using YAML files and CLI flags.
tags: ['configuration', 'yaml', 'setup', 'gateway']
---

Source: https://docs.datzi.ai/gateway/configuration
...
<Tip>
  **New to configuration?** Start with `datzi onboard` for interactive setup, or
  check out the [Configuration Examples](/gateway/configuration-examples) guide
...
<Tab title="CLI (one-liners)">
  ```bash datzi config get agents.defaults.workspace datzi config set
  agents.defaults.heartbeat.every "2h" datzi config unset
  tools.web.search.apiKey ```
</Tab>

<Tab title="Control UI">
  Open [http://127.0.0.1:18789](http://127.0.0.1:18789) and use the **Config**
  tab. The Control UI renders a form from the config schema, with a **Raw JSON**
  editor as an escape hatch.
</Tab>

  <Tab title="Direct edit">
    Edit `~/.datzi/datzi.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
