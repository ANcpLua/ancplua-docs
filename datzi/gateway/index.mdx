...
summary: High-level runbook for operating, starting, and managing a Datzi Gateway.
tags: ['runbook', 'operations', 'overview', 'gateway']
---

Source: https://docs.datzi.ai/gateway/index
...
<Note>
  ...
  profile/state defaults, or `OPENCLAW_CONFIG_PATH` when set). Default mode is
...
- Auth is required by default (`gateway.auth.token` / `gateway.auth.password`, or `OPENCLAW_GATEWAY_TOKEN` /
...
```bash
OPENCLAW_CONFIG_PATH=~/.datzi/a.json OPENCLAW_STATE_DIR=~/.datzi-a datzi gateway --port 19001
OPENCLAW_CONFIG_PATH=~/.datzi/b.json OPENCLAW_STATE_DIR=~/.datzi-b datzi gateway --port 19002
```
