...
Source: https://docs.datzi.ai/gateway/configuration-reference
...
- `capabilities`: optional list (`image`, `audio`, `video`). Defaults: `openai`/`anthropic`/`minimax` → image,`google` →
...
```bash
OPENCLAW_CONFIG_PATH=~/.datzi/a.json \
OPENCLAW_STATE_DIR=~/.datzi-a \
...
- Hostname defaults to `datzi`. Override with `OPENCLAW_MDNS_HOSTNAME`.
