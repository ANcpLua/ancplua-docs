...
slug: gateway/doctor
title: Doctor
summary: Documents the `datzi doctor` diagnostic command for troubleshooting Gateway health issues.
tags: ['doctor', 'diagnostics', 'troubleshooting', 'gateway']
---

Source: https://docs.datzi.ai/gateway/doctor

# Doctor

`datzi doctor` is the repair + migration tool for Datzi. It fixes stale
...
```bash
datzi doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
datzi doctor --non-interactive
```

Run without prompts and only apply safe migrations (config normalization + on-disk state moves). Skips
restart/service/sandbox actions that require human confirmation.
...
- Model auth health: checks OAuth expiry, can refresh expiring tokens, and reports auth-profile cooldown/disabled
  states.
- Extra workspace dir detection (`~/datzi`).
...
you to run `datzi doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.datzi/datzi.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.datzi/sessions/` to `~/.datzi/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.datzi/agent/` to `~/.datzi/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.datzi/credentials/*.json` (except `oauth.json`)
  - to `~/.datzi/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `datzi doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.datzi` folders exist across
  home directories or when `DATZI_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.datzi/datzi.json` is
...
offers to remove them and install the Datzi service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Datzi gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `datzi doctor --generate-gateway-token` to force token
...
- `datzi doctor` prompts before rewriting supervisor config.
- `datzi doctor --yes` accepts the default repair prompts.
- `datzi doctor --repair` applies recommended fixes without prompts.
- `datzi doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `datzi gateway install --force`.
