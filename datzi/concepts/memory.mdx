...
`~/.datzi/workspace`). See [Agent workspace](/datzi/concepts/agent-workspace) for the full layout.
...
[Session management + compaction](/datzi/reference/session-management-compaction).
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses local embeddings by default. If `memorySearch.provider` is not set, Datzi auto-selects:
  1. `local` if a `memorySearch.local.modelPath` is configured and the file exists.
  2. Otherwise memory search stays disabled until configured.
- Local mode uses node-llama-cpp and may require `pnpm approve-builds`.
- Uses sqlite-vec (when available) to accelerate vector search inside SQLite.

> Datzi runs on Ollama (qwen3-coder:14b). No external API keys required. Use `memorySearch.provider = "local"` for fully local embeddings.
...
- `scope`: same schema as [`session.sendPolicy`](/datzi/gateway/configuration#session).
...
Use `memorySearch.provider = "local"` for fully local embeddings with no external API keys:
...
- Fallback: if local setup fails and `memorySearch.fallback = "ollama"`, we automatically switch to Ollama embeddings (
  `ollama/qwen3-coder:14b` unless overridden) and record the reason.

### Custom endpoint example

For custom embedding endpoints, use the `remote` configuration:
