...
Source: https://docs.datzi.ai/concepts/multi-agent
...
available from `~/.datzi/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- Config: `~/.datzi/datzi.json` (or `OPENCLAW_CONFIG_PATH`)
- State dir: `~/.datzi` (or `OPENCLAW_STATE_DIR`)
...
- Workspace defaults to `~/.datzi/workspace` (or `~/.datzi/workspace-<profile>` when `OPENCLAW_PROFILE` is set).
