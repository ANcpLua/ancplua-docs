...
Goal: multiple _isolated_ agents (separate workspace + `agentDir` + sessions), plus multiple channel accounts (e.g. two
WhatsApps) in one running Gateway. Inbound is routed to an agent via bindings.
...
available from `~/.datzi/skills`. See [Skills: per-agent vs shared](/datzi/tools/skills#per-agent-vs-shared-skills).
...
<Steps>
  ...
  <Step title="Create channel accounts">
    ...
    * WhatsApp: link each phone number per account.
    ...
    See channel guides: [Discord](/datzi/channels/discord), [Telegram](/datzi/channels/telegram), [WhatsApp](/datzi/channels/whatsapp).
...
You can route **different WhatsApp DMs** to different agents while staying on **one WhatsApp account**. Match on sender
E.164 (like `+15551234567`) with `peer.kind: "direct"`. Replies still come from the same WhatsApp number (no per‑agent
sender identity).

Important detail: direct chats collapse to the agent’s **main session key**, so true isolation requires **one agent per
person**.
...
- For shared groups, bind the group to one agent or use [Broadcast groups](/datzi/channels/broadcast-groups).
...
If a binding sets multiple match fields (for example `peer` + `guildId`), all specified fields are required (`AND`
semantics).
...
- If you have multiple accounts for a channel, add `accountId` to the binding (for example
  `{ channel: "whatsapp", accountId: "personal" }`).
- To route a single DM/group to Opus while keeping the rest on chat, add a `match.peer` binding for that peer; peer
  matches always win over channel-wide rules.
...
See [Multi-Agent Sandbox & Tools](/datzi/tools/subagents) for detailed examples.
