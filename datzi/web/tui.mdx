...
summary: Documents the Terminal UI (TUI) for interacting with the Datzi Gateway.
tags: [tui, terminal, gateway]
---

Source: https://docs.datzi.ai/web/tui
...
```bash
datzi tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `datzi tui --deliver`
...
- Check the Gateway logs: `datzi logs --follow`.
- Confirm the agent can run: `datzi status` and `datzi models status`.
...
- No agents in picker: check `datzi agents list` and your routing config.
