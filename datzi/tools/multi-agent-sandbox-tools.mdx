...
id: DATZI-271
category: tools
slug: multi-agent-sandbox-tools
title: Multi-Agent Sandbox & Tools Configuration
summary: Covers per-agent sandbox and tool restriction configuration for multi-agent setups, including config examples, precedence rules, tool groups, elevated mode, migration from single-agent, and troubleshooting.
tags:
  [multi-agent, sandbox, tools, policy, docker, agents, security, restriction]
---

Source: https://docs.datzi.ai/tools/multi-agent-sandbox-tools
...
For debugging "why is this blocked?",
see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `datzi sandbox explain`.
...
- `agents.list[].sandbox.{docker,browser,prune}.*` overrides `agents.defaults.sandbox.{docker,browser,prune}.*` for that
  agent (ignored when sandbox scope resolves to `"shared"`).
...
`tools.elevated` is the global baseline (sender-based allowlist). `agents.list[].tools.elevated` can further restrict
elevated for specific agents (both must allow).
...
Legacy `agent.*` configs are migrated by `datzi doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=datzi-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.datzi}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
