...
slug: exec
title: Exec tool
summary: Covers the `exec` tool for running shell commands in the workspace, including parameters, config, PATH handling, session overrides, authorization, exec approvals, allowlists, and the experimental apply_patch subtool.
tags:
  [exec, shell, sandbox, approvals, process, background, security, allowlist]
---
...
- `elevated` (bool): request elevated mode (gateway host); `security=full` is only forced when elevated resolves to
  `full`
...
- `gateway`/`node` approvals are controlled by `~/.datzi/exec-approvals.json`.
...
- `tools.exec.notifyOnExit` (default: true): when true, backgrounded exec sessions enqueue a system event and request a
  heartbeat on exit.
- `tools.exec.approvalRunningNoticeMs` (default: 10000): emit a single "running" notice when an approval-gated exec runs
...
- `tools.exec.safeBins`: stdin-only safe binaries that can run without explicit allowlist entries. For behavior details,
  see [Safe bins](/datzi/tools/exec#allowlist--safe-bins).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Datzi prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
See below for the policy, allowlist, and UI flow details.
