...
summary: Covers the Datzi Chrome MV3 extension that lets the agent control existing Chrome tabs via a local CDP relay, including installation, token setup, attach/detach, remote gateway setup, sandboxing, and security implications.
tags: [browser, chrome, extension, relay, cdp, sandbox, security]
---

Source: https://docs.datzi.ai/tools/chrome-extension

# Chrome extension (browser relay)

The Datzi Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of
launching a separate datzi-managed Chrome profile.
...
- **Local relay server** (loopback CDP): bridges between the control server and the extension (`http://127.0.0.1:18792`
  by default)
- **Chrome MV3 extension**: attaches to the active tab using `chrome.debugger` and pipes CDP messages to the relay

Datzi then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Datzi release (npm package) as static files. There is no separate "build" step.

After upgrading Datzi:

- Re-run `datzi browser extension install` to refresh the installed files under your Datzi state directory.
- Chrome → `chrome://extensions` → click "Reload" on the extension.

## Use it (set gateway token once)

Datzi ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `DATZI_GATEWAY_TOKEN`)
...
- Open the tab you want Datzi to control.
...
- `ON`: attached; Datzi can drive that tab.
- `…`: connecting to the local relay.
- `!`: relay not reachable/authenticated (most common: relay server not running, or gateway token missing/wrong).

If you see `!`:

- Make sure the Gateway is running locally (default setup), or run a node host on this machine if the Gateway runs
  elsewhere.
...
Debugging: `datzi sandbox explain`
...
`datzi browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `datzi browser extension install` first to
copy the extension to a stable location under your Datzi state directory.
...
- **This is not isolated** like the dedicated datzi-managed profile.
