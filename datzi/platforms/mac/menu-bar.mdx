...
summary: Documents the macOS menu bar icon state model, session priority rules, activity kinds, and debug override options for agent work visualization.
tags: [macos, menu-bar, icon, sessions, agent, swift, debug]
---
...
- Sessions: events arrive with `runId` (per-run) plus `sessionKey` in the payload. The "main" session is the key `main`;
  if absent, we fall back to the most recently updated session.
- Priority: main always wins. If main is active, its state is shown immediately. If main is idle, the most recently
  active non‑main session is shown. We do not flip‑flop mid‑activity; we only switch when the current session goes idle
  or main becomes active.
- Activity kinds:
...
- While work is active: `<Session role> · <activity label>`
  - Examples: `Main · exec: pnpm test`, `Other · read: apps/macos/Sources/Datzi/AppState.swift`.
