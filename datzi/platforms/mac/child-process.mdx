...
summary: Describes how the macOS app manages the Gateway via launchd (not as a child process), including attach-only and remote modes.
tags: [macos, gateway, launchd, lifecycle, remote, child-process]
---
...
service via the external `datzi` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.datzi.*` is supported).
