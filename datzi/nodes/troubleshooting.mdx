...
summary: Diagnostic runbook for node connectivity and capability failures, covering foreground requirements, permissions, pairing vs approvals, and common error codes.
tags:
  [
    nodes,
    troubleshooting,
...
Source: https://docs.datzi.ai/nodes/troubleshooting
...
```bash
datzi nodes describe --node <idOrNameOrIp>
datzi nodes canvas snapshot --node <idOrNameOrIp>
datzi logs --follow
```
...
datzi nodes status
datzi approvals get --node <idOrNameOrIp>
datzi approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
