...
failover). For runtime diagnostics, see [Troubleshooting](/datzi/gateway/troubleshooting). For the full config reference,
see [Configuration](/datzi/gateway/configuration).
...
- [Model failover and "All models failed"](#model-failover-and-all-models-failed)
  ...
  - [Why did it also try another provider and fail?](#why-did-it-also-try-another-provider-and-fail)
...
4. **Deep probes**
   ...
   Runs gateway health checks + provider probes (requires a reachable gateway). See [Health](/datzi/gateway/health).
...
5. **Tail the latest log**
   ...
   File logs are separate from service logs; see Logging and [Troubleshooting](/datzi/gateway/troubleshooting).
...
6. **Run the doctor (repairs)**
   ...
   Repairs/migrates config/state + runs health checks. See [Doctor](/datzi/gateway/troubleshooting).
...
7. **Gateway snapshot**
   ...
   Asks the running gateway for a full snapshot (WS-only). See [Health](/datzi/gateway/health).
...
[https://github.com/datzi/issues](https://github.com/datzi/issues)
[https://github.com/datzi/pulls](https://github.com/datzi/pulls)
...
Install docs: [Install](/datzi/install/index), [Installer flags](/datzi/install/installer), [Updating](/datzi/install/updating).
...
local screen/camera/canvas or command execution. See [Nodes](/datzi/nodes/index).
...
Docs: Linux, [Install](/datzi/install/index).
...
is pointed at the right Gateway. See [Remote access](/datzi/gateway/remote).
...
Related: [Migrating](/datzi/install/migrating), [Where things live on disk](/datzi/help/faq#where-does-datzi-store-its-data),
[Agent workspace](/datzi/concepts/agent-workspace), [Doctor](/datzi/gateway/troubleshooting),
[Remote mode](/datzi/gateway/remote).

### Where do I see what is new in the latest version

Check the GitHub changelog:
[https://github.com/datzi/blob/main/CHANGELOG.md](https://github.com/datzi/blob/main/CHANGELOG.md)
...
detail: [Troubleshooting](/datzi/help/troubleshooting#docsdatziai-shows-an-ssl-error-comcastxfinity).
...
[https://github.com/datzi/tree/main/docs](https://github.com/datzi/tree/main/docs)
...
More detail: Development channels and [Installer flags](/datzi/install/installer).
...
If it hangs, use [Installer stuck](/datzi/help/faq#installer-stuck-how-do-i-get-more-feedback)
and the fast debug loop in [Im stuck](/datzi/help/faq#im-stuck--whats-the-fastest-way-to-get-unstuck).
...
More options: [Installer flags](/datzi/install/installer).
...
More detail: [Install](/datzi/install/index) and [Installer flags](/datzi/install/installer).
...
- Full walkthrough: [Getting Started](/datzi/start/getting-started).
- Installer + updates: [Install & updates](/datzi/install/updating).
...
Remote access: [Gateway remote](/datzi/gateway/remote).
...
Hub: Platforms. Remote access: [Gateway remote](/datzi/gateway/remote).
Nodes: [Nodes](/datzi/nodes/index), Nodes CLI.
...
Docs: Update, [Updating](/datzi/install/updating).
...
Docs: [Local models](/datzi/gateway/local-models), [Models](/datzi/concepts/models).
...
Yes, with a sufficiently large local model. Datzi works well with Ollama models like `qwen3-coder:32b` or `deepseek-r1:32b`. Use the largest model your hardware supports. See [Security](/datzi/gateway/authentication).

More context: [Models](/datzi/concepts/models).
...
Docs: [Local models](/datzi/gateway/local-models),
[Model providers](/datzi/concepts/model-providers), [Security](/datzi/gateway/authentication),
[Sandboxing](/datzi/gateway/sandboxing).
...
Docs: [Models](/datzi/concepts/models), Configure, Config, [Doctor](/datzi/gateway/troubleshooting).

### What do Datzi, Flawd, and Krill use for models

- **Datzi + Flawd:** Ollama (`ollama/qwen3-coder:32b`) - see [Local models](/datzi/gateway/local-models).
- **Krill:** MiniMax M2.1 (`minimax/MiniMax-M2.1`) - see [Local models](/datzi/gateway/local-models).
...
See [Models](/datzi/concepts/models) and [Slash commands](/datzi/tools/slash-commands).
...
See [Local models](/datzi/gateway/local-models) and [Models](/datzi/concepts/models).
...
Docs: [Models](/datzi/concepts/models), [Multi-Agent Routing](/datzi/concepts/multi-agent), [Model providers](/datzi/concepts/model-providers).
...
### Why did it also try another provider and fail

If your model config includes another provider as a fallback, Datzi will try it
during model fallback. If you haven't configured credentials for that provider, you'll see a "No API key found" error.

Fix: remove external provider models from `agents.defaults.model.fallbacks` / aliases and use local Ollama models instead (e.g. `ollama/qwen3-coder:14b`).
...
- Still stuck? Run `datzi status --all` and follow [Troubleshooting](/datzi/gateway/troubleshooting).
  See Dashboard for auth details.
...
- macOS: `$DATZI_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.datzi/logs/...`; profiles use
  `~/.datzi-<profile>/logs/...`)
...
See [Troubleshooting](/datzi/gateway/troubleshooting#log-locations) for more.
...
If you run the gateway manually, `datzi gateway --force` can reclaim the port. See [Gateway](/datzi/gateway/index).
...
Docs: Windows (WSL2), [Gateway service runbook](/datzi/gateway/index).
...
Docs: [Channels](/datzi/channels/index), [Troubleshooting](/datzi/gateway/troubleshooting), [Remote access](/datzi/gateway/remote).
...
Docs: Dashboard, [Remote access](/datzi/gateway/remote), [Troubleshooting](/datzi/gateway/troubleshooting).
...
Docs: [Telegram](/datzi/channels/telegram), [Channel troubleshooting](/datzi/channels/troubleshooting).
...
Docs: TUI, [Slash commands](/datzi/tools/slash-commands).
...
Docs: [Gateway service runbook](/datzi/gateway/index).
...
See [Datzi assistant setup](/datzi/start/getting-started) and [Agent send](/datzi/tools/subagents).
...
Details: [Security](/datzi/gateway/authentication).
...
Docs: [Security](/datzi/gateway/authentication), [Pairing](/datzi/channels/pairing).
...
[Security](/datzi/gateway/authentication).
...
tools and run inside a sandbox. See [Security](/datzi/gateway/authentication).
...
Docs: [Thinking and verbose](/datzi/tools/thinking), [Security](/datzi/gateway/authentication).
...
Slash commands overview: see [Slash commands](/datzi/tools/slash-commands).
...
a [GitHub discussion](https://github.com/datzi/discussions).
