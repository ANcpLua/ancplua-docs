...
```bash
pnpm gateway:dev
OPENCLAW_PROFILE=dev datzi tui
```
...
1. **Profile isolation** (global `--dev`)
   ...
   - `OPENCLAW_STATE_DIR=~/.datzi-dev`
   - `OPENCLAW_CONFIG_PATH=~/.datzi-dev/datzi.json`
...
```bash
OPENCLAW_PROFILE=dev datzi gateway --dev --reset
```
...
OPENCLAW_RAW_STREAM_PATH=~/.datzi/logs/raw-stream.jsonl
```
